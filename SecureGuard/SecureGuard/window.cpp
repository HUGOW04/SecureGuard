    ...
    : m_Width(width), m_Height(height), m_Title(title)
{
    ...
    m_FullscanImage = std::make_unique<Loadimage>("images/fullscan.png");
    m_FastscanImage = std::make_unique<Loadimage>("images/fastscan.png");
    ...
    scanButtons.emplace_back(
        280, 140, 110, 30,
        "Fullscan", "Fullscan",
        m_Italic.get(), m_FullscanImage.get(),
        Color(0.2196f, 0.2549f, 0.3137f),   // normal
        Color(0.3f, 0.35f, 0.4f),           // hover
        Color(0.1f, 0.7f, 0.6f)             // selected
    );

    scanButtons.emplace_back(
        280, 190, 110, 30,
        "Filescan", "Filescan",
        m_Italic.get(), m_FilescanImage.get(),
        Color(0.2196f, 0.2549f, 0.3137f),
        Color(0.3f, 0.35f, 0.4f),
        Color(0.1f, 0.7f, 0.6f)
    );

    scanButtons.emplace_back(
        400, 140, 110, 30,
        "Fastscan", "Fastscan",
        m_Italic.get(), m_FastscanImage.get(),
        Color(0.2196f, 0.2549f, 0.3137f),
        Color(0.3f, 0.35f, 0.4f),
        Color(0.1f, 0.7f, 0.6f)
    );
    ...
    scanButtons.emplace_back(
        400, 190, 110, 30,
        "Memoryscan", "Memscan",
        m_Italic.get(), m_MemoryscanImage.get(),
        Color(0.2196f, 0.2549f, 0.3137f),
        Color(0.3f, 0.35f, 0.4f),
        Color(0.1f, 0.7f, 0.6f)
    );
...
void Window::drop_callback(GLFWwindow* window, int count, const char** paths)
{
    ...
    // Only allow drop if the Scan panel is active
    if (!win->scan) return;
    ...
    // Check if mouse is inside drop rectangle
        ...
        mouseY >= rectY && mouseY <= rectY + rectHeight)
    {
        for (int i = 0; i < count; ++i)
        {
            ...
            // Send file to Filescan
            std::string hash = win->m_Scan->fileToSHA256(filepath.string());
            if (win->m_Scan->compareSHA256Str(hash))
            {
                std::cout << "Warning: File is in SHA-256 virus database! -> " << filepath << std::endl;
                std::ofstream threatFile("logs/threat_log.txt", std::ios::app);
                threatFile << "[SHA-256 MATCH] " << filepath << std::endl;
                threatFile.close();

                win->m_Scan->setThreatBuffer("[SHA-256 MATCH] " + filepath.string());
            }

            win->m_Scan->setLogBuffer(filepath.string());
        }
    }
    else
    {
        std::cout << "File dropped outside of drop area, ignoring." << std::endl;
    }
}
...
void Window::handleEvents()
{
    ...
    while (!glfwWindowShouldClose(m_Window))
    {
        ...
        else if (scan)
        {
            ...
            m_DropImage->drawImage(650.0f, 160.0f, 120.0f, 120.0f);
