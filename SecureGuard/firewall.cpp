...
Firewall::Firewall(Font* title, Font* text, std::vector<std::string>& settingsTitle, std::vector<std::string>& settingsDescriptions, std::vector<Toggle>& toggle)
...
void Firewall::drawCorner(float cx, float cy, float radius, int startAngle, int endAngle) {
...
void Firewall::drawRoundedRect(float x, float y, float width, float height, float radius) {
    float right = x + width;
