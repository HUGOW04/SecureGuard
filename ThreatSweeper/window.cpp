    ...
    : m_Width(width), m_Height(height), m_Title(title)
{
    ...
    scanButtons.push_back(Button(180, 110, "Fullscan", "fullscan", m_Italic.get()));
    scanButtons.push_back(Button(330, 110, "Fastscan", "fastscan", m_Italic.get()));
    scanButtons.push_back(Button(180, 160, "Singlescan", "singlescan", m_Italic.get()));
    scanButtons.push_back(Button(330, 160, "Memoryscan", "memoryscan", m_Italic.get()));

    m_Sidebar = std::make_unique<Sidebar>(sidebarButtons);
    m_Overview = std::make_unique<Overview>(overviewWidget);
    m_ScanPanel = std::make_unique<Scanpanel>(scanButtons);
}
    ...
    if (win->scan)
    {
        for (auto& b : win->scanButtons)
        {
            if (mouseX >= b.getX() && mouseX <= b.getX() + b.Width() &&
                mouseY >= b.getY() && mouseY <= b.getY() + b.Height())
            {
                if (b.getName() == "Fullscan")
                {
                    std::cout << "clicked on fullscan!" << std::endl;
                    std::thread([win]() {
                        win->m_Scan->recursiveIterator();
                        }).detach();

                    
                }
                else if (b.getName() == "Fastscan")
                {
                    std::cout << "clicked on Fastscan!" << std::endl;
                }
                else if (b.getName() == "Singlescan")
                {
                    std::cout << "clicked on Singlescan!" << std::endl;
                }
                else if (b.getName() == "Memoryscan")
                {
                    std::cout << "clicked on Memoryscan!" << std::endl;
                    std::thread([win]() {
                        win->m_Scan->ScanMemory();
                        }).detach();
