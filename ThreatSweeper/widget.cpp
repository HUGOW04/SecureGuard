...
void Widget::render()
{
    ...
    // --- Etikett (större text, top left) ---

    // --- Värdetext (mindre, centrerad under) ---
    if (m_ValueFont && m_ValueFont->isValid() && m_ValueText)
    {
        ...
        m_ValueFont->render(*m_ValueText, centerX, valueY, 1.0f); // Mindre
    }
    ...
    if (m_LabelFont && m_LabelFont->isValid())
    {
        ...
        m_LabelFont->render(m_LabelText, labelX, labelY, 1.0f); // Större
    }
}
